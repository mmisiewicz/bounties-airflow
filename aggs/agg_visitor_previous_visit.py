...
YYDY = """{{ (execution_date - macros.timedelta(days = 1)).strftime("%Y") }}"""
YYDM = """{{ (execution_date - macros.timedelta(days = 1)).strftime("%m") }}"""
YYDD = """{{ (execution_date - macros.timedelta(days = 1)).strftime("%d") }}"""
...
WHERE
    time_received >= '%(lhdy)s-%(lhdm)s-%(lhdd)s 23:00:00'
"""
...
insert_cmd = """INSERT INTO agg_visitor_previous_visit (user_id_uuid, user_id, has_wallet, last_visit, pageviews, ymd)
...
all_past_pageviews as (select user_id_uuid, user_id, max(date(time_received)) as ymd from staging.user_page_log
                       ...
                       group by user_id_uuid, user_id)
select daily_pageviews.user_id_uuid, daily_pageviews.user_id, '%(YDY)s-%(YDM)s-%(YDD)s' as ymd, has_wallet, pageviews,
